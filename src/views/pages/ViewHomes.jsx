...
const ViewHomes = () => {
  ...
  const decode_user_id = base64_decode(homes_id);
  const [show, setShow] = useState(false);
  const [homes_data, setHomesDetails] = useState([]);
  const [enlargedImage, setEnlargedImage] = useState(null);
  const [showImagePopup, setShowImagePopup] = useState(false);

  const handleClose = () => setShow(false);
  const handleShow = () => setShow(true);
  console.log('erferferf : ', homes_id);
  var fetchData = () => {
    console.log('homes_id : ', homes_id);

    axios
      .get(`${API_URL}get_homes_details/${decode_user_id}`)
      .then((response) => {
        if (response.data.success) {
          setHomesDetails(response.data.homes_arr[0]);
          console.log('setTransaction', response.data.homes_arr[0]);
        } else {
