...
const BusinessReportedList = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .get(`${API_URL}get_tabular_business?from_date=${from_date}&to_date=${to_date}`)
      .then((response) => {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'BusinessReport');
    const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([excelBuffer], { type: 'application/octet-stream' });
    saveAs(blob, 'BusinessReport.xlsx');
  };
  return (
    <>
      <div className="col-xl-12" style={{ backgroundColor: '#FFF', borderRadius: '12px', padding: '10px', marginBottom: '20px' }}>
        ...
        >
          Manage Business Tabular Reports
        </p>
      </div>
      <MainCard title="Business Report" className="mb-3">
              ...
              <Paper sx={{ width: '100%' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div>
                    ...
                    <button onClick={() => handleChangePage(null, page + 1)} disabled={(page + 1) * rowsPerPage >= filteredUsers.length}>
                      {'>'}
                    </button>
                  </div>
                </div>
...
export default BusinessReportedList;
