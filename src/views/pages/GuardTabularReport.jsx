...
const GuardTabularReport = () => {
  ...
  const handleView = (user_id) => {
    ...
    navigate(APP_PREFIX_PATH + `/view-guard/${encode_user_id}`);
  };
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .get(`${API_URL}get_tabular_guard?from_date=${from_date}&to_date=${to_date}`)
      .then((response) => {
        if (response.data.success) {
          setUserData(response.data.user_arr);
          console.log("Data ABHi :", response.data.user_arr);
  ...
  return (
    ...
    <div className="col-xl-12" style={{ backgroundColor: '#FFF', borderRadius: '12px', padding: '10px', marginBottom: '20px' }}>
        ...
        >
          Manage Guard Tabular Reports
        </p>
      </div>
      <MainCard title="Guard Report" className="mb-3">
...
export default GuardTabularReport;
