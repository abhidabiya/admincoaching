...
const columns = [
    ...
    { id: 'society', label: 'Society Name', align: 'center' },
    { id: 'wing_name', label: 'Building Name', align: 'center' },
    // { id: 'wing_type', label: 'Wing Type', align: 'center' },
...
const ManageBranches = () => {
    ...
    var fetchData = () => {
        axios
            .get(`${API_URL}get_all_bulidng`)
            .then((response) => {
                setfaqData(response.data.building_arr);
            })
            .catch((error) => {
    ...
    const handleAction = (action, faqData) => {
        if (action === 'Edit') {
            ...
            setSelectedIndex(null);
            setEditQuestion(faqData.building_name);
            setEditAnswer(faqData.answer);
            setFaqDelete(faqData.building_id);
    ...
    const deleteFAQ = () => {
        axios
            .post(`${API_URL}delete_building`, { building_id: faqToDelete })
            .then((response) => {
    ...
    const handleEdit = (e) => {
        ...
        if (!editQuestion) {
            setEditQuestionError('Please Enter Building');
            hasError = true;
        } else {
        ...
        data.append('building_id', faqToDelete);
        data.append('building_name', editQuestion);
        // data.append('answer', editAnswer);

        axios
            .post(`${API_URL}edit_building`, data)
            .then((response) => {
    ...
    const handleAdd = (e) => {
        ...
        if (!addQuestion) {
            setAddQuestionError('Please Enter Building');
            hasError = true;
        } else {
            setAddQuestionError('');
        }
        ...
        const data = new FormData();
        data.append('building_name', addQuestion);
        // data.append('answer', addAnswer);
        axios
            .post(`${API_URL}add_building`, data)
            .then((response) => {
                if (response.data.key === 'FaqAlreadyExist') {
                    setAddQuestionError('Building Already Exists');
                } else {
                    setShowModal2(false);
                    fetchData();
                    setAddQuestion('');
    ...
    const filteredUsers = faq_data.filter((user) => {
        ...
        const buildingMatch = user.building_name?.toLowerCase().includes(lowercasedTerm);
        const societyMatch = user.society_name?.toLowerCase().includes(lowercasedTerm);
        ...
        return buildingMatch || dateMatch || societyMatch;
    });
        ...
        <>
            ...
            <Box alignItems="center" justifyContent="space-between" display="flex" className="mobile-res">
                ...
                {/* <Button className="btn btn-primary "
                    // onClick={() => navigate(APP_PREFIX_PATH + "/add-wing")} 
                            ...
                            {filteredUsers.length > 0 ? (
                                filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
                                    <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                                        ...
                                        <TableCell style={{ textAlign: 'center' }}>{row.society_name}</TableCell>
        ...
        /> */}
                ...
                >
                    <Modal.Header closeButton>
                        <Modal.Title style={{ fontSize: '17px' }}>Add Building</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                ...
                >
                    <Modal.Header closeButton>
                        <Modal.Title style={{ fontSize: '17px' }}>Edit Building</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <form>
                            <div className="mb-3">
